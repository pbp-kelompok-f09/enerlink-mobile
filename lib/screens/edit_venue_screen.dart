...
class _EditVenueScreenState extends State<EditVenueScreen> {
  ...
  Future<void> _loadVenueData() async {
    try {
      final venues = await CreateVenueService.getVenues();
      final venue = venues.data?.firstWhere(
        (v) => v.idVenue == widget.venueId,
      ...
      if (venue != null) {
        setState(() {
          ...
          _contactPersonController.text = venue.contactPerson;
          _sessionPriceController.text = venue.sessionPrice.toStringAsFixed(0);
          _ratingController.text = venue.rating.toStringAsFixed(1);
          _imageUrlController.text = venue.imageUrl;
          
          if (venue.category.isNotEmpty) {
            final category = venue.category.first;
            if (_commonCategories.contains(category)) {
          ...
          if (venue.facility.isNotEmpty) {
            _facilityController.text = venue.facility.join(', ');
          }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error loading venue: ${e.toString()}'),
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      await CreateVenueService.updateVenue(
